        ...
        <table className="w-full text-left border-collapse table-fixed">
          <thead className="bg-gray-50 border-b border-gray-200 text-gray-500 text-[12px]">
            <tr>
              ...
              {/* 키워드 너비 30%로 조정 (유사도 칼럼 공간 확보 위해) */}
              <th className="px-4 py-4 font-bold w-[30%]">키워드</th>
              <th className="px-2 py-4 font-bold text-center w-[15%]">유사도</th>
              <th className="px-2 py-4 font-bold text-right w-[15%]">PC</th>
              <th className="px-2 py-4 font-bold text-right w-[15%]">모바일</th>
              <th className="px-4 py-4 font-bold text-right bg-blue-50/50 text-blue-700 w-[15%]">합계</th>
            </tr>
          </thead>
            ...
            {currentItems.map((item: any, i: number) => {
              ...
              return (
                <tr key={i} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td onClick={() => onKeywordClick(item.relKeyword)} className="px-4 py-4 font-bold text-gray-800 cursor-pointer hover:text-blue-600 hover:underline truncate" title={item.relKeyword}>
                    {item.relKeyword}
                  </td>
                  <td className="px-2 py-4 text-center truncate">
                    <span className={`font-bold ${item.similarity === '높음' ? 'text-red-600' : 'text-green-600'}`}>
                      {item.similarity}
                    </span>
                  </td>
                  <td className="px-2 py-4 text-right text-gray-400 truncate">{pc.toLocaleString()}</td>
                  <td className="px-2 py-4 text-right text-gray-400 truncate">{mo.toLocaleString()}</td>
