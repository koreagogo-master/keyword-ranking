...
interface SearchResult {
  ...
  isMainExposed: boolean | null; // 통검 노출 여부 (null은 로딩전/에러)
  title: string;
}
...
export default function KinRankPage() {
  ...
  // [안전 장치] 랜덤 대기 함수 (최소 min ms ~ 최대 max ms)
  ...
  const handleCheck = async () => {
    ...
    for (let i = 0; i < validInputs.length; i++) {
      ...
      // 진행 상태 표시 (휴식 시간 포함 안내)
      if (i > 0) {
          setProgress(`안전한 검색을 위해 잠시 대기 중... (${i + 1}/${validInputs.length})`);
          // [안전 장치] 2초 ~ 4초 랜덤 휴식 (로봇 차단 방지)
