...
export async function checkNaverBlogRank(keyword: string, targetNicknames: string): Promise<RankResult> {
  try {
    ...
    return { success: false, message: `오류 발생: ${error.message || error}`, reason: 'ERROR' };
  }
}

export async function checkNaverBlogRankDeep(keyword: string, targetNicknames: string): Promise<RankResult> {
  try {
    const cookieStore = await cookies();
    const supabase = createServerClient(
      process.env.NEXT_PUBLIC_SUPABASE_URL!,
    ...
    try {
        ...
        await browser.close();
        return { success: false, message: '네이버 접속 시간 초과 (프록시 재시도 필요)', reason: 'ERROR' };
    }

    // 딥서치는 15번 스크롤하여 50위까지 탐색 (blog-rank 와 동일)
    for (let i = 0; i < 15; i++) { 
      await page.evaluate(() => window.scrollBy(0, 800));
    ...
    const foundItems = await page.evaluate((targets) => {
      ...
      for (const dateItem of uniqueDates) {
          ...
          const normalizedAuthor = cleanStringInBrowser(author);
          if (targets.some((t: string) => normalizedAuthor === t)) {
            myRankings.push({ rank: currentRank, title: title || '제목 없음', author: author, date: cleanDate, url: url, section: '블로그탭(딥)' });
          }
          if (currentRank >= 50) break; // 🌟 50위까지만 탐색
      }
      return myRankings; 
    }, targets);
