...
export async function POST(request: Request) {
    try {
        ...
        // ============================================================
        // [본 처리] 위 검증을 통과한 경우에만 무료 횟수 수정 실행
        // ============================================================
        const { userId, newVal } = await request.json();

        if (!userId) {
            return NextResponse.json({ error: '대상 유저 ID가 필요합니다.' }, { status: 400 });
        }

        // [보안 3단계] newVal 유효성 검증: 0 이상의 정수만 허용
        if (typeof newVal !== 'number' || isNaN(newVal) || newVal < 0 || !Number.isInteger(newVal)) {
            return NextResponse.json({ error: '유효하지 않은 값입니다. 0 이상의 정수를 입력해주세요.' }, { status: 400 });
        }

        // service_role 클라이언트로 free_search_count만 업데이트
        ...
        if (updateError) {
            return NextResponse.json({ error: `무료 횟수 수정 실패: ${updateError.message}` }, { status: 500 });
        }
        ...
        console.error('무료 횟수 수정 에러:', error);
